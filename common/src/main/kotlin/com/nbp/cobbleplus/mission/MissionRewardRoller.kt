...
object MissionRewardRoller {
    ...
    fun roll(rewards: List<RewardEntry>, rolls: Int, random: Random = Random.Default): List<RewardRollResult> {
        if (rewards.isEmpty() || rolls <= 0) return emptyList()
        val results = mutableListOf<RewardRollResult>()
        for (i in 0 until rolls) {
            val hits = rewards.filter { random.nextDouble() * 100.0 < it.chance }
            if (i == 0 && hits.isEmpty()) {
                val forced = rewards.random(random)
